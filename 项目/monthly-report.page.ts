...
export class MonthlyReportPage implements OnInit {
  ...
  getData(error?){
    this.queryFlag = true;
    if(this.isShowForm){
    if(!!this.maxVal && (typeof this.maxVal)!=="number" ){
      this.dataObj.cParentUUID = null;
      this.queryFlag = false
      this.toast.show("最大值输入有误！")
    }else if(!!this.minVal &&  (typeof this.minVal)!=="number"){
      this.dataObj.cParentUUID = null;
      this.queryFlag = false
      this.toast.show("最小值输入有误！")
    }else if ((typeof this.maxVal)==="number" && (typeof this.minVal)==="number" && this.maxVal >= 0 && this.minVal >= 0 && this.maxVal >= this.minVal) {
      this.dataObj.cParentUUID = this.minVal + ',' + this.maxVal;
    } else if((!this.maxVal && this.minVal) || (this.maxVal && !this.minVal) || (this.maxVal < this.minVal) || this.maxVal<0 || this.minVal<0){
      this.dataObj.cParentUUID = null;
      this.queryFlag = false
      this.toast.show("数值输入有误！")
    }
    }
    ...
    .subscribe(
      (res: any) => {
        ...
        if (res.code === 1) {
          ...
          this.listOfDisplayData = this.listOfDisplayData.filter(i => true)
  ...
  ngOnInit() {
    // location.reload() 
      // console.log("monthly:",this.nav.pop(this.nav.lastNavId)
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if(this.listOfDisplayData.length > 0){
        for(let i=0;i<this.listOfDisplayData.length;i++){
          if(this.listOfDisplayData[i].project.projectUUID === this.PUUID && this.listOfDisplayData[i].yueBao === "未上报"){
            this.listOfDisplayData[i].yueBao = "未审核"
            this.listOfDisplayData = this.listOfDisplayData.filter(i => true)
          }
        }
        // console.log(this.YB,this.PUUID,this.listOfDisplayData)
      }
    });
    // this.historyL = window.history.length;
    // this.hlBK = this.historyL;
