...
@Component({
  selector: 'app-market-value-target-indexmain',
  templateUrl: './indexmain.component.html',
})
export class MarketValueTargetIndexmainComponent extends MarketValueTargetImplementReportComponent {
  ...
  uploadFiles() {
    ...
    if (this.idxData.bEnd && (!this.idxData.iIndexXz2 || this.idxData.iIndexXz2.length === 0)) {
      this.msg.warning('请选择考核对象！');
      return;
    }
  ...
  getChecked() {
    ...
    if (this.depFlag === 1) {
      this.idxData.iIndexXz2.forEach(i => {
        let id: any;
        this.depList.forEach((j, index) => {
          j.cOrgName === i.cOrgName ? (id = index) : null;
        });
  ...
  checkIn() {
    ...
    if (this.depFlag === 1) {
      this.depExam = tmp.join(',');
      this.idxData.iIndexXz2 = [];
      tmpDep.forEach(item => {
        this.idxData.iIndexXz2.push({
          cOrgUUID: item.cOrgUUID, //  "部门UUID(组织)",
          cOrgName: item.cOrgName, //  "部门名称(组织)",
          cOrgCode: item.cOrgCode, //  "部门编号(组织)"
        });
